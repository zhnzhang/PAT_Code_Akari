//2020.02.10 还不能独立作答这道题，倒是能临摹默写出来标准答案。
//这道题难在程序的实现逻辑，核心代码部分
//首先要分节处理，其次是对0的处理很重要，另外如果这一节没有非零的可以输出，也要作标记去进行处理
//单碰到0是不输出的，而是置标记为true，这样就能合适处理连续0的情况
//一旦碰到非零的，flag显示是否之前有累积的0 再选择是否进行输出，就能达成题意
//对空格的控制也有些讲究，这里使用" %d"会更顺利
//参数的设定自行推算，个位和个节都要单独判定不对其进行wei的输出
